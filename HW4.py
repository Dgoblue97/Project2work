...
html_file = "nytimes_data.html"
try:
	New_york_times_data = open(html_file,'r')
	NYT_data = New_york_times_data.read()

except:
	NYT_requests = requests.get("http://www.nytimes.com")
	NYT_data = NYT_requests.text
	New_york_times_data = open(html_file,'w')
	New_york_times_data.write(NYT_data)
...
soup = BeautifulSoup(NYT_data, 'html.parser')
...
NYT_Headlines = []
for new_heading in soup.find_all(class_="story-heading"): 
    if new_heading.a: 
        NYT_Headlines.append(new_heading.a.text.replace("\n", " ").strip())
    else:
    	NYT_Headlines.append((new_heading.contents[0].strip()))    

nytimes_headlines = NYT_Headlines[0:10]
for a in range(10):
	print (nytimes_headlines[a])	
